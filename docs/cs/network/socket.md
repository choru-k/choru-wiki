...
int setup_optimized_listener(int server_fd) {
    ...
    printf("Server listening with backlog %d (system max)\n", max_backlog);
    return 0;
}
```
...
void performance_comparison_test() {
    ...
    printf("Unix Domain Socket: %.3f seconds\n", unix_time);
    printf("TCP Socket (localhost): %.3f seconds\n", tcp_time);
    printf("Unix Domain Socket is %.2fx faster\n", tcp_time / unix_time);
}
```
...
int tune_socket_buffers(int sockfd) {
    ...
    printf("Socket buffers tuned to %d bytes\n", optimal_size);
    return 0;
}
