...
1. **코드영역**: 공유되고 읽기 전용이므로 여러 프로세스가 효율적으로 사용
2. **데이터영역**: 전역 상태를 저장하며, BSS로 초기화 비용 절감
3. **스택**: 스레드별로 독립적이며, 크기 제한에 주의 필요
4. **힙**: 동적 할당 영역으로 단편화와 메모리 누수 주의
5. **메모리 매핑**: 대용량 파일과 공유 메모리에 효율적
