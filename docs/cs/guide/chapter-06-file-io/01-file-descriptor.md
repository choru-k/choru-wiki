...
// 이건 각 프로세스가 가지는 "내 파일들" 목록입니다
struct files_struct {
    atomic_t count;              // 참조 카운트
    bool resize_in_progress;
    wait_queue_head_t resize_wait;
    
    struct fdtable __rcu *fdt;  // 실제 파일 디스크립터 테이블
    struct fdtable fdtab;        // 기본 테이블
    
    spinlock_t file_lock;        // 테이블 보호 락
    unsigned int next_fd;        // 다음 할당할 fd 힌트
    unsigned long close_on_exec_init[1];  // exec 시 닫을 fd 비트맵
    unsigned long open_fds_init[1];       // 열린 fd 비트맵
    unsigned long full_fds_bits_init[1];  // 가득 찬 fd 비트맵
    struct file __rcu *fd_array[NR_OPEN_DEFAULT];  // 파일 포인터 배열
};

// 2. 파일 디스크립터 테이블
struct fdtable {
    unsigned int max_fds;        // 최대 fd 개수
    struct file __rcu **fd;      // 파일 포인터 배열
    unsigned long *close_on_exec;  // exec 시 닫을 fd 비트맵
    unsigned long *open_fds;        // 열린 fd 비트맵  
    unsigned long *full_fds_bits;  // 가득 찬 비트맵 섹션
    struct rcu_head rcu;
};
...
struct file {
    ...
    struct inode           *f_inode;     // 캐시된 inode
    const struct file_operations *f_op;  // 파일 연산 테이블
    
    spinlock_t             f_lock;
    enum rw_hint           f_write_hint;
    atomic_long_t          f_count;      // 참조 카운트
    unsigned int           f_flags;      // O_RDONLY, O_NONBLOCK 등
    fmode_t                f_mode;       // FMODE_READ, FMODE_WRITE 등
    struct mutex           f_pos_lock;
    loff_t                 f_pos;        // 현재 파일 위치
    struct fown_struct     f_owner;      // 시그널 전송용 소유자 정보
    const struct cred     *f_cred;       // 파일 열 때의 자격 증명
    struct file_ra_state   f_ra;         // read-ahead 상태
    
    u64                    f_version;
    void                  *f_security;    // LSM 보안 정보
    void                  *private_data;  // 드라이버/파일시스템 전용
    
    struct list_head       f_ep_links;    // epoll 백링크
