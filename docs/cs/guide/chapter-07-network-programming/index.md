...
### [7-5: TCP 연결 디버깅](05-tcp-connection-debugging.md)

**"연결 문제의 원인을 찾아서"**

- 🔍 **연결 상태 분석**: netstat, ss로 소켓 상태 모니터링
- 🐛 **TIME_WAIT 문제**: 대량 연결 처리 시 발생하는 이슈 해결
- 📊 **패킷 분석**: tcpdump/Wireshark를 활용한 네트워크 디버깅
- ⚡ **성능 최적화**: 연결 재사용과 풀링 전략
- 🔧 **커널 파라미터 튜닝**: 네트워크 스택 최적화

### [7-6: 네트워크 지연 최적화](06-network-latency-optimization.md)

**"마이크로초 단위의 최적화"**

- ⏱️ **레이턴시 측정**: 정확한 네트워크 지연 측정 기법
- 🚀 **Nagle 알고리즘**: TCP_NODELAY와 패킷 전송 최적화
- 💾 **버퍼링 전략**: 송수신 버퍼 크기 최적화
- 🌐 **지리적 최적화**: CDN과 엣지 컴퓨팅 활용
- 📈 **성능 프로파일링**: 네트워크 병목지점 식별

### [7-7: 고성능 네트워킹 기법](07-high-performance-networking.md)

**"극한의 처리량을 위한 최적화"**

- ⚡ **Zero-copy 네트워킹**: sendfile(), splice() 활용
- 🔧 **io_uring**: 최신 비동기 I/O 인터페이스
- 🏗️ **DPDK**: 커널 우회 패킷 처리
- 📊 **RDMA**: 원격 메모리 직접 접근
- 🌐 **분산 네트워킹**: 로드 밸런싱과 failover 전략
