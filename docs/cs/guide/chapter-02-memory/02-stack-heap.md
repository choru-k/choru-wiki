...
여러분이 음료수를 사려고 합니다. 두 가지 방법이 있습니다:

1. **자판기 (스택)**: 버튼만 누르면 음료가 나옵니다. 빠르고 간단하지만, 정해진 종류만 있고 수량도 제한적입니다.

2. **창고 (힙)**: 창고 관리인에게 요청하면 원하는 것을 찾아줍니다. 다양한 선택이 가능하지만, 시간이 걸리고 직접 반납해야 합니다.

프로그래밍의 스택과 힙도 정확히 이런 차이를 보입니다. 스택은 자판기처럼 빠르고 자동화되어 있지만 제한적입니다. 힙은 창고처럼 유연하지만 관리가 필요합니다.

이 장에서는 이 두 메모리 영역이 어떻게 우리의 프로그램을 지탱하는지, 그리고 각각의 장단점은 무엇인지 깊이 있게 살펴보겠습니다.
