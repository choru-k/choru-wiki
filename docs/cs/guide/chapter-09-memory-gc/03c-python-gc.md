...
# Chapter 9-3c: Python GC - Reference Counting의 이중주

## 🎯 이 문서를 읽고 나면 얻을 수 있는 것들

이 문서를 마스터하면, 여러분은:

1. **"Python은 왜 GC가 느릴까요?"** - Reference Counting과 Cycle Detection의 특성을 이해합니다
2. **"GIL이 GC에 어떤 영향을 미치나요?"** - Python의 멀티스레딩 한계와 메모리 관리의 관계를 파악합니다
3. **"Python 메모리가 자꾸 늘어나요"** - 메모리 누수 패턴과 프로파일링 기법을 배웁니다
4. **"실제 서비스에서 Python GC를 어떻게 최적화하나요?"** - Instagram, Dropbox 등의 실제 최적화 사례를 익힙니다
