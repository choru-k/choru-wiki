...
void demonstrate_nice_effect() {
    ...
    for (int nice = -20; nice <= 19; nice += 5) {
        ...
        if (pid == 0) {
            ...
            printf("Nice %3d: Count = %lu, ", nice, count);
            exit(0);
        }
    }
...
// CPU별 전용 스레드 생성 - 고성능 병렬 처리의 핵심 패턴
// 실제 사용 예: Nginx 워커 프로세스, Redis I/O 스레드, 게임 엔진 렌더링
void create_per_cpu_threads() {
    // ⭐ 1단계: 시스템의 CPU 코어 개수 확인
    // _SC_NPROCESSORS_ONLN: 현재 온라인 상태인 CPU 코어 수 반환
    // HyperThreading 환경에서는 논리 코어 개수 (물리 코어 * 2)
    int num_cpus = sysconf(_SC_NPROCESSORS_ONLN);
    pthread_t threads[num_cpus];
    
    printf("Creating %d worker threads for %d CPU cores\n", num_cpus, num_cpus);
    
    // ⭐ 2단계: 각 CPU 코어별로 전용 워커 스레드 생성
    // 1:1 매핑으로 컨텍스트 스위칭 최소화 및 캐시 효율성 극대화
    for (int i = 0; i < num_cpus; i++) {
        // CPU 인덱스를 void* 포인터로 전달 (표준 pthread 방식)
        int *cpu_id = malloc(sizeof(int));
        *cpu_id = i;
        
        if (pthread_create(&threads[i], NULL, 
                          worker_thread, cpu_id) != 0) {
            fprintf(stderr, "Failed to create thread for CPU %d\n", i);
            continue;
        }
        
        printf("Created worker thread for CPU %d\n", i);
    }
    
    // ⭐ 3단계: 모든 워커 스레드가 완료될 때까지 대기
    for (int i = 0; i < num_cpus; i++) {
...
// CPU별 전용 워커 스레드 - 최적화된 CPU 바인딩 구현
void* worker_thread(void *arg) {
    int cpu = *(int*)arg;
    free(arg);  // 동적 할당된 메모리 해제
    
    printf("Worker thread started for CPU %d (PID: %d, TID: %ld)\n", 
           cpu, getpid(), pthread_self());
    
    // ⭐ 1단계: 스레드를 특정 CPU 코어에 강제 바인딩
    // 이렇게 하면 OS가 이 스레드를 다른 코어로 이동시키지 않음
    set_cpu_affinity(cpu);
    
    // 바인딩 확인: 실제로 해당 CPU에서 실행되는지 검증
    int actual_cpu = sched_getcpu();
    if (actual_cpu == cpu) {
        printf("✅ Thread successfully bound to CPU %d\n", cpu);
    } else {
        printf("⚠️ Thread binding failed! Expected CPU %d, got %d\n", cpu, actual_cpu);
    }
    
    // ⭐ 2단계: CPU별 로컬 데이터 구조 초기화
    // L1/L2 캐시 최적화를 위한 스레드 로컬 작업 공간 설정
    thread_local_data_t *local_data = init_thread_local_data(cpu);
    
    // ⭐ 3단계: CPU 집약적 작업 루프 - 캐시 친화적 처리
    int work_cycles = 0;
    clock_t start_time = clock();
    
    while (!stop_flag) {
        // CPU 친화도를 활용한 최적화된 작업 처리
        // - L1 캐시 히트율 극대화
        // - 메모리 접근 패턴 최적화
        // - NUMA 노드 지역성 활용
        cpu_intensive_work_optimized(local_data);
        
        work_cycles++;
        
        // 주기적 성능 통계 출력 (1초마다)
        if (work_cycles % 1000000 == 0) {
            clock_t current_time = clock();
            double elapsed = ((double)(current_time - start_time)) / CLOCKS_PER_SEC;
            
            printf("CPU %d: Processed %d cycles in %.2f seconds (%.0f cycles/sec)\n",
                   cpu, work_cycles, elapsed, work_cycles / elapsed);
        }
        
        // CPU 바인딩 지속성 확인 (디버깅용)
        if (work_cycles % 10000 == 0) {
            int current_cpu = sched_getcpu();
            if (current_cpu != cpu) {
                printf("⚠️ CPU %d thread migrated to CPU %d!\n", cpu, current_cpu);
                // 필요시 다시 바인딩
                set_cpu_affinity(cpu);
            }
        }
    ...
    // ⭐ 4단계: 스레드 정리 및 성능 결과 출력
    clock_t end_time = clock();
    double total_time = ((double)(end_time - start_time)) / CLOCKS_PER_SEC;
    
    printf("CPU %d worker completed: %d cycles in %.2f seconds (avg: %.0f cycles/sec)\n",
           cpu, work_cycles, total_time, work_cycles / total_time);
    
    cleanup_thread_local_data(local_data);
    return NULL;
}
...
void print_numa_info() {
    ...
    for (int node = 0; node < num_nodes; node++) {
        ...
        printf("  Memory: %ld MB, ", size / (1024*1024));
...
void measure_scheduling_latency() {
    ...
    printf("Scheduling latency: %ld ns, ", latency_ns);
