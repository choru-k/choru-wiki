...
### 1️⃣ [인터럽트 기초와 개념](./02-03-interrupt-basics.md)
...
### 2️⃣ [인터럽트 처리 과정과 예외](./02-13-interrupt-processing.md)
...
### 3️⃣ [인터럽트 컨트롤러와 최적화](./02-14-interrupt-controllers.md)
...
### 4️⃣ [소프트 인터럽트와 실시간 처리](./02-15-software-interrupts.md)
...
1. [기초와 개념](./02-03-interrupt-basics.md) → 인터럽트가 무엇인지 이해
2. [처리 과정](./02-13-interrupt-processing.md) → 실제 동작 메커니즘 학습
3. 간단한 인터럽트 핸들러 작성 연습

### 중급자 (심화 학습)

1. [컨트롤러와 최적화](./02-14-interrupt-controllers.md) → 성능 최적화 기법
2. [실시간 처리](./02-15-software-interrupts.md) → 고급 디버깅과 튜닝
...
- [CPU 아키텍처](./02-01-cpu-architecture.md) - Protection Ring과 실행 모드
- [메모리 관리](../../chapter-02-memory/) - 가상 메모리와 페이지 폴트

### 후속 학습  

- [컨텍스트 스위칭](./02-16-context-switching.md) - 인터럽트 기반 프로세스 전환
...
**시작하기**: [인터럽트 기초와 개념](./02-03-interrupt-basics.md)에서 인터럽트의 필요성과 기본 동작을 학습합니다.
