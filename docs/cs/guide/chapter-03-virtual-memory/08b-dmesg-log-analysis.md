...
dmesg에서 OOM 이벤트 파싱
    ...
    def analyze_oom_patterns(self):
        ...
        for event in self.oom_events:
            ...
            if time_diff < 86400:  # 24시간 내 이벤트 표시
            ...
            print(f"    익명 RSS: {event.memory_info['anon_rss'] / 1024:.1f} MB (힐, 스택 등 실제 메모리 사용량)")
    ...
    def generate_recommendations(self):
        ...
        if len(self.oom_events) > 5:
            ...
            print("   - 스왈 설정 검토 (vm.swappiness, zram/zswap 활용)")
        ...
        print("   - 컴테이너 메모리 제한 활용 (Docker --memory, Kubernetes resources.limits)")
...
OOM 발생 시 dmesg에 상세한 정보가 기록되며, 이를 통해 희생양 선택 과정과 메모리 사용 패턴을 분석할 수 있습니다.

### 2. 패턴 분석

반복적으로 OOM되는 프로세스를 식별하여 메모리 누수 가능성을 진단할 수 있습니다.

### 3. 자동 분석 도구

Python 스크립트를 통해 OOM 이벤트를 자동으로 파싱하고 분석할 수 있습니다.

---

**이전**: [08a-oom-fundamentals.md](08a-oom-fundamentals.md)  
